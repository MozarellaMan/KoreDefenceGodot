...
namespace KoreDefenceGodot.Core.Scripts.Engine.Tiles
{
    ...
    public class Path
    {
        ...
        /// </summary>
        public int[,] PathPoints { get; private set; } = null!;
        ...
        public void Setup(Tile[,] tiles, int[] horizontalPaths, int[] verticalPaths, int tileSize = 40)
        {
            ...
            for (var i = 0; i < horzLength; i++)
            {
                ...
                OccupyTileWithPath(Tiles, GetCorner(horizontalPaths[i], verticalPaths[i], true), newPositionX,
                    newPositionX, _curPositionY, _curPositionY);
                ...
                if (i + 1 < horzLength)
                    OccupyTileWithPath(Tiles, GetCorner(horizontalPaths[i + 1], verticalPaths[i], false), _curPositionX,
                        _curPositionX, newPositionY, newPositionY);
                else
                    break;
            }
        ...
        private static PathType GetCorner(int horizontal, int vertical, bool isTravellingHorizontally)
        {
            ...
            if (horizontal > 0) return vertical > 0 ? PathType.RightDown : PathType.DownLeft;
